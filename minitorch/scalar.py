...
def central_difference(f, *vals, arg=0, epsilon=1e-6):
    ...
    """
    listRep = list(vals)  # Actually faster than concat for high val lengths
    listRep[arg] += epsilon / 2.   # and scalability is important here.
    vals = tuple(listRep)
    high = f(*vals)
    listRep = list(vals)
    listRep[arg] -= epsilon
    ...
    # Concat would have been better for code conciseness/code golf
